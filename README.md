...
A personal hub for your static mini-sites. Each top-level folder in this repo is a
...
- `build.js` scans the repo root for subfolders.
  - Folders starting with `.` or `_`, plus `node_modules`, are ignored.
...
- The whole repo root is published, so each mini-site is reachable at `/<folder>/`.
...
1. Copy the [`_template/`](_template/) folder and rename it (the folder name becomes the URL path,
   e.g. `my-tool/` → `/my-tool/`).
